({
            ...
            if(navigator.onLine){
                ...
                action.setCallback(this, function (response) {
                    if (response.getState() === "SUCCESS") {
                        ...
                        var name = component.find("dispName").get("v.value");
    ...
    Author:        Bryant Daniels
    Company:       Slalom, LLC
    Description:   runs validation check on input fields
    History
    <Date>      <Authors Name>     <Brief Description of Change>
    5/14/2017    Bryant Daniels     Initial Creation
    ------------------------------------------------------------*/
    ...
    Author:        Bryant Daniels
    Company:       Slalom, LLC
    Description:   validation for input fields on component
    History
    <Date>      <Authors Name>     <Brief Description of Change>
    5/14/2017    Bryant Daniels     Initial Creation
    ------------------------------------------------------------*/
    validatePage: function (component, event) {
        try{
            ...
            var dispDate = component.find('dt').get("v.value");
            var subjects = [];
            var dates = [];
            var status = component.find("statusSelectOptions").get("v.value");

            if (objective) {
                this.inputValidityCheck(component, component.find("dispName")) ? null : errorsFound = true;
                ...
                if(component.get("v.statusDeclined")){
                    if(component.find("dispQtyD")){
                        component.set("v.newObjective.Product_Quantity__c", "0");
                    }
                   }
               /* if(component.find("dispQty")){
                ...
                var suggestedDatedetails = component.find('dt').get("v.value");
                var today = new Date(); 
                var formatteddate =  new Date(suggestedDatedetails);
                if(status == 'Committed'){
                    if (!suggestedDatedetails) {
                        if($A.get("$Browser.formFactor") == "DESKTOP"){
                        ...
                        if($A.get("$Browser.formFactor") == "DESKTOP"){
                            this.displayToast($A.get("$Label.c.Warning"),  $A.get("$Label.c.Suggested_Delivery_Date_Check"), 'warning');
                        } else {
                            this.displayToastMobile(component, $A.get("$Label.c.Warning"), $A.get("$Label.c.Suggested_Delivery_Date_Check"), 'slds-theme_warning');
                        }
                        errorsFound = true;
                        return;
                    }
    ...
    Author:        Bryant Daniels
    Company:       Slalom, LLC
    Description:   navigates to call log after save of objective
    History
    <Date>      <Authors Name>     <Brief Description of Change>
    5/14/2017    Bryant Daniels     Initial Creation
    ------------------------------------------------------------*/
    navigateToCallLog : function(component, event){
        ...
        if($A.get("$Browser.formFactor") == "DESKTOP"){
            ...
            // MC:1701- Subtypes should be added on objectives instead of planned objectives
            component.set("v.subType","");
            component.set("v.newRecord", component.find("dispName").get("v.value"));
    ...
    setFormFieldsFromTemplate : function(component, template) {
        try{
            var emptyArray = [];
            if(template != null){
                component.find("dispName").set("v.value", template.Name);
                component.find("dispDetails").set("v.value", template.Description__c);
                component.find("dispQty").set("v.value", template.Product_Quantity__c);
                var brandSearch = component.find("brandSearch");
                var offPrem = component.get("v.dispIsOffPremise");
