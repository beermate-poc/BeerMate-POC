({
    ...
    Company:       Slalom, LLC
    Description:   creates objective record
    History
    <Date>      <Authors Name>     <Brief Description of Change>
    5/14/2017    Bryant Daniels     Initial Creation
    ------------------------------------------------------------*/
    createMerchandise: function (component, event) {
        try{
            if(navigator.onLine){
                ...
                action.setCallback(this, function (response) {
                    if (response.getState() === "SUCCESS") {
                        ...
                        var name = component.find("merchandiseName").get("v.value");
    ...
    Author:        Bryant Daniels
    Company:       Slalom, LLC
    Description:   validates input fields on the objective page
    History
    <Date>      <Authors Name>     <Brief Description of Change>
    5/14/2017    Bryant Daniels     Initial Creation
    ------------------------------------------------------------*/
    validatePage: function (component, event) {
        try{
            ...
            if (objective) {
                this.inputValidityCheck(component, component.find("merchandiseName")) ? null : errorsFound = true;
            ...
            if (!errorsFound) {
                this.createMerchandise(component, event);
            }
        } catch(e){
            console.error(e);
            if($A.get("$Browser.formFactor") == $A.get("$Label.c.DESKTOP")){
    ...
    navigateToCallLog : function(component, event){
        ...
        if($A.get("$Browser.formFactor") == "DESKTOP"){
            ...
            // MC:1701- Subtypes should be added on objectives instead of planned objectives
            component.set("v.subType","");
            component.set("v.newRecord", component.find("merchandiseName").get("v.value"));
    ...
    setFormFieldsFromTemplate : function(component, template) {
        try{
            var emptyArray = [];
            if(template != null){
                component.find("merchandiseName").set("v.value", template.Name);
                component.find("merchandiseDetail").set("v.value", template.Description__c);
