({
    ...
    defaultValues : function(component, event) {
        //alert('default')
    ...
    setFormFieldsFromTemplate : function(component, template,helper) {
        try{
            ...
            if(template != null){
                ...
                if(isCreate || (component.get("v.newObjective.BMC_Product_Level__c") == $A.get("$Label.c.BMC_Objectives_Brand")) || (component.get("v.newObjective.BMC_Product_Level__c") == $A.get("$Label.c.BMC_Objectives_BrandPackage"))){
                    component.find("EngbrandSearch").set("v.searchValue", template.Brand_Name__c);
                }
                if(!isCreate && ((component.get("v.newObjective.BMC_Product_Level__c") == $A.get("$Label.c.BMC_Objectives_BrandGroupPackage")) || (component.get("v.newObjective.BMC_Product_Level__c") == $A.get("$Label.c.BMC_Objectives_BrandGroup")))){
                    component.find("EngbrandSearch").set("v.searchValue", template.BMC_Brand_Group__c);
                }
                if(!isCreate && ((component.get("v.newObjective.BMC_Product_Level__c") == $A.get("$Label.c.BMC_Brand_Family_Package")) )){
                    component.find("EngbrandSearch").set("v.searchValue", template.BMC_Brand_Family__c);
                }
                
                if(template.Product_Quantity__c){
                    component.set("v.newObjective.Product_Quantity__c", template.Product_Quantity__c);
                }
                if(isCreate || (component.get("v.newObjective.BMC_Product_Level__c") == $A.get("$Label.c.BMC_Brand_Family_Package")) || (component.get("v.newObjective.BMC_Product_Level__c") == $A.get("$Label.c.BMC_Objectives_BrandGroupPackage")) || (component.get("v.newObjective.BMC_Product_Level__c") == $A.get("$Label.c.BMC_Objectives_BrandPackage"))){
    ...
    onSaveHelper : function(component, event){
            ...
            if(reqFieldsMap){
                for(var i in reqFieldsMap){
                    ...
                    if((reqFieldsMap[i].value != undefined && reqFieldsMap[i].value != null) && (component.get("v.newObjective."+reqFieldsMap[i].key)==undefined || component.get("v.newObjective."+reqFieldsMap[i].key)=="" || component.get("v.newObjective."+reqFieldsMap[i].key)== null )){
                        //if(reqFieldsMap[i].key == 'Brands__c' && (component.get("v.newObjective.Status__c") == 'Committed' || component.get("v.newObjective.Status__c") == 'Executed')){
                            //error = true;
        ...
        var objective = component.get("v.newObjective");
            ...
            if(!component.get("v.isCreate")){
                ...
                objective.Sub_Type__r = null;
                objective.BMC_Product_Level__c=$A.get("$Label.c.BMC_Objectives_BrandPackage");
            }
        //alert(JSON.stringify(objective));
